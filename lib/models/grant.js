...
module.exports = (provider) => class Grant extends apply([
  ...
  getOIDCScope() {
    console.log('***** en getOIDCScope')
    console.log(this.openid)
    console.log(this.openid?.scope)
    if (this.openid && this.openid.scope) {
      console.log('getOIDCScope: this.rejected: ' + this.rejected)
      if (this.rejected) {
        const rejected = this.getOIDCScope.call(this.rejected).split(' ');
        console.log('getOIDCScope: rejected = ')
        console.log(rejected)
        const granted = new Set(this.openid.scope.split(' '));
        console.log('getOIDCScope: granted: ' + JSON.stringify(granted))
        for (const scope of rejected) {
          if (scope !== 'openid') {
            granted.delete(scope);
          }
        }
        console.log('getOIDCScope: granted length: ' + granted.size)
        console.log('getOIDCScope: granted returned: ')
        console.log(granted)
        return [...granted].join(' ');
      }
      console.log('getOIDCScope: returning: ' + this.openid.scope)
      return this.openid.scope;
    }
    console.log('getOIDCScope: returning empty')
    return '';
  }
  ...
  getOIDCScopeFiltered(filter) {
    console.log('**** en getOIDCScopeFiltered')
    console.log(filter)
    ...
    console.log('getOIDCScopeFiltered: granted returned: ' + JSON.stringify(granted))
